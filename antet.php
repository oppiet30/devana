...
$db_host="localhost"; $db_user="oppie"; $db_pass="20KiNg73"; $db_name="devana";
...
function menu_up()
{
	...
	if (isset($_SESSION["user"][1], $_GET["town"]))
	{
		...
		$map_lnk="<a class='q_link' href='map.php?x=0&y=0'>".$lang['map']."</a>";
	echo "<a class='q_link' href='index.php'>".$lang['home']."</a> | ";
	if (!isset($_SESSION["user"][1]))
		echo "<a class='q_link' href='login.php'>".$lang['login']."</a> | 
...
function menu_down()
{
	...
	if ($alert[0][0])
		$alert[0]="<font color='red'>".$alert[0][0]."</font> "; else $alert[0]="";
	if ($alert[1][0])
		$alert[1]="<font color='red'>".$alert[1][0]."</font> "; else $alert[1]="";
	if (isset($_SESSION["user"][1]))
		echo "<a class='q_link' href='reports.php?page=0'>".$alert[0].$lang['reports']."</a> | <a class='q_link' href='messages.php?page=0'>".$alert[1].$lang['messages']."</a> || <a class='q_link' href='chat.php'>".$lang['chat']."</a> | ";
	if ((isset($_SESSION["user"][4]))&&($_SESSION["user"][4]>3))
		echo "<a class='q_link' href='apanel.php'>".$lang['adminPanel']."</a> | ";
}
