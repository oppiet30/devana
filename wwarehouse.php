...
if ($data[21])
{
	...
	if (!$c_status[21])
		if ($data[21]<10)
		{
			...
			echo $tag;
			if ($town[12]+$town[3]+$upk[$data[21]]>$lim[3])
				label("</br>".$lang['noHouses']);
				if (!(($res[0]>=$cost[0]*pow($r, $data[21]))&&($res[1]>=$cost[1]*pow($r, $data[21]))&&($res[2]>=$cost[2]*pow($r, $data[21]))&&($res[3]>=$cost[3]*pow($r, $data[21]))&&($res[4]>=$cost[4]*pow($r, $data[21]))))
					label("</br>".$lang['noResources']);
			echo "</br>------------------------------------------</br>";
		}
		else
			echo $lang['buildingMaxLvl']."</br></br></br></br></br></br>";
	else
		echo $lang['beingUpgraded']."</br></br></br></br></br></br>";
}
else
